import 'package:customer/equipment/data/model/response/basic_campaign_model.dart';
import 'package:customer/equipment/data/model/response/item_model.dart';
import 'package:customer/equipment/data/model/response/store_model.dart';

class BannerModel {
  List<BasicCampaignModel>? campaigns;
  List<BannerModel1>? banners;

  BannerModel({this.campaigns, this.banners});

  BannerModel.fromJson(Map<String, dynamic> json) {
    if (json['campaigns'] != null) {
      campaigns = [];
      json['campaigns'].forEach((v) {
        campaigns!.add(BasicCampaignModel.fromJson(v));
      });
    }
    if (json['banners'] != null) {
      banners = [];
      json['banners'].forEach((v) {
        banners!.add(BannerModel1.fromJson(v));
      });
    }
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = <String, dynamic>{};
    if (campaigns != null) {
      data['campaigns'] = campaigns!.map((v) => v.toJson()).toList();
    }
    if (banners != null) {
      data['banners'] = banners!.map((v) => v.toJson()).toList();
    }
    return data;
  }
}

class BannerModel1 {
  int? id;
  String? title;
  String? sub_title;
  String? type;
  String? image;
  String? link;
  Store? store;
  Item? item;


  String? bg_color;
   int? is_main;
  int? module_id;





  BannerModel1({this.id, this.title, this.type, this.image,this.link, this.store, this.item});

  BannerModel1.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    title = json['title'];
    type = json['type'];
    image = json['image'];
    sub_title = json['sub_title'];
    link = json['link'];
    bg_color = json['bg_color'];
    is_main = json['is_main'];
    module_id = json['module_id'];
    store = json['store'] != null ? Store.fromJson(json['store']) : null;
    item = json['item'] != null ? Item.fromJson(json['item']) : null;
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = <String, dynamic>{};
    data['id'] = id;
    data['title'] = title;
    data['type'] = type;
    data['image'] = image;
    data['link'] = link;
    data['bg_color'] = bg_color;
    data['sub_title'] = sub_title;
    data['is_main'] = is_main;
    data['module_id'] = module_id;
    if (store != null) {
      data['store'] = store!.toJson();
    }
    if (item != null) {
      data['item'] = item!.toJson();
    }
    return data;
  }
}
