...
class CategoryRepo {
  ...
  Future<Response> getSubCategoryList(
    String? parentID,
  ) async {
  ...
  Future<Response> getCategoryItemList(
    ...
    print("version_idversion_idversion_idversion_id ${version_id.runtimeType}");
    return await apiClient.getData(
        '${AppConstants.categoryItemUri}$categoryID?limit=10&offset=$offset&type=$type&year=${year ?? ""}&version_id=${(version_id.toString() == "null" ? null : version_id) ?? ""}');
  }

  Future<Response> getCareVersionList(String? categoryID) async {
    return await apiClient.getData(
        '${AppConstants.careVersionsItemUri}?category_id=${categoryID ?? ""}',
