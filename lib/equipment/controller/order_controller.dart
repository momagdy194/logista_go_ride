...
import 'package:customer/equipment/data/model/response/distance_model.dart';
import 'package:customer/equipment/data/model/response/order_cancellation_body.dart';
...
import 'package:customer/equipment/data/model/response/refund_model.dart';
...
import 'package:customer/equipment/data/model/response/timeslote_model.dart';
...
import 'package:customer/equipment/helper/network_info.dart';
...
class EQOrderController extends GetxController implements GetxService {
  final OrderRepo orderRepo;
  EQOrderController({required this.orderRepo});
  ...
  void clearPrevData(int? zoneID) {
    ...
    _acceptTerms = true;
    try {
      ZoneData zoneData = Get.find<EQLocationController>().getUserAddress()!.zoneData!.firstWhere((element) => element.id == zoneID);
      _paymentMethodIndex = zoneData.cashOnDelivery! ? 0 : zoneData.digitalPayment! ? 1
          : Get.find<EQSplashControllerEquip>().configModel!.customerWalletStatus == 1 ? 2 : 0;
    }catch(e) {
      _paymentMethodIndex = Get.find<EQSplashControllerEquip>().configModel!.cashOnDelivery! ? 0
          : Get.find<EQSplashControllerEquip>().configModel!.digitalPayment! ? 1
          : Get.find<EQSplashControllerEquip>().configModel!.customerWalletStatus == 1 ? 2 : 0;
    }
    _selectedDateSlot = 0;
    _selectedTimeSlot = 0;
  ...
  Future<void> initializeTimeSlot(Store store) async {
    ...
    for(int index=0; index<store.schedules!.length; index++) {
      ...
      if(minutes > Get.find<EQSplashControllerEquip>().configModel!.scheduleOrderSlotDuration!) {
        DateTime time = openTime;
        for(;;) {
          if(time.isBefore(closeTime)) {
            DateTime start = time;
            DateTime end = start.add(Duration(minutes: Get.find<EQSplashControllerEquip>().configModel!.scheduleOrderSlotDuration!));
            ...
            time = time.add(Duration(minutes: Get.find<EQSplashControllerEquip>().configModel!.scheduleOrderSlotDuration!));
          }else {
            break;
          }
        }
  ...
  void validateSlot(List<TimeSlotModel> slots, int dateIndex, int? interval, {bool notify = true}) {
    _timeSlots = [];
    DateTime now = DateTime.now();
    if(Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.orderPlaceToScheduleInterval!) {
