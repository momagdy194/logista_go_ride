...
class EQItemController extends GetxController implements GetxService {
  final ItemRepo itemRepo;
  EQItemController({required this.itemRepo});
  ...
  int setExistInCart(Item? item, {bool notify = false}) {
    ...
    if (!Get.find<EQSplashControllerEquip>()
        .getModuleConfig(Get.find<EQSplashControllerEquip>().module != null
            ? Get.find<EQSplashControllerEquip>().module!.moduleType
            : Get.find<EQSplashControllerEquip>().cacheModule!.moduleType)
        .newVariation!) {
    ...
    if (Get.find<EQSplashControllerEquip>()
      ...
      _cartIndex = Get.find<EQCartController>()
          .isExistInCart(item!.id, variationType, false, null);
    }
    if (_cartIndex != -1) {
      _quantity = Get.find<EQCartController>().cartList[_cartIndex].quantity;
      _addOnActiveList = [];
      _addOnQtyList = [];
      List<int?> addOnIdList = [];
      for (var addOnId
          in Get.find<EQCartController>().cartList[_cartIndex].addOnIds!) {
        addOnIdList.add(addOnId.id);
      }
      for (var addOn in item!.addOns!) {
        if (addOnIdList.contains(addOn.id)) {
          _addOnActiveList.add(true);
          _addOnQtyList.add(Get.find<EQCartController>()
              .cartList[_cartIndex]
