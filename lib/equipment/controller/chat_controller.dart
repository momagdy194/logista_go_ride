...
import 'package:customer/equipment/data/model/response/chat_model.dart';
import 'package:customer/equipment/data/repository/chat_repo.dart';
...
class EQChatController extends GetxController implements GetxService {
  final ChatRepo chatRepo;
  EQChatController({required this.chatRepo});
      ...
      if(index0 != -1) {
        ...
        if(sender) {
          _conversationModel!.conversations![index0]!.sender = User(
            id: 0, fName: Get.find<EQSplashControllerEquip>().configModel!.businessName, lName: '',
            phone: Get.find<EQSplashControllerEquip>().configModel!.phone, email: Get.find<EQSplashControllerEquip>().configModel!.email,
            image: Get.find<EQSplashControllerEquip>().configModel!.logo,
          );
        }else {
      ...
      if (offset == 1) {
        ...
        if(_messageModel!.conversation == null) {
          _messageModel!.conversation = Conversation(sender: User(
            id: Get.find<EQUserController>().userInfoModel!.id, image: Get.find<EQUserController>().userInfoModel!.image,
            fName: Get.find<EQUserController>().userInfoModel!.fName, lName: Get.find<EQUserController>().userInfoModel!.lName,
  ...
  Future<Response?> sendMessage({required String message, required NotificationBody? notificationBody,
    ...
    if (response!.statusCode == 200) {
      ...
      if(Get.find<EQUserController>().userInfoModel!.userInfo == null) {
        Get.find<EQUserController>().updateUserWithNewData(_messageModel!.conversation!.sender);
      }
      _sortMessage(notificationBody!.adminId);
