...
class EQCartController extends GetxController implements GetxService {
  final CartRepo cartRepo;
  EQCartController({required this.cartRepo});
    ...
    if (Get.find<EQSplashControllerEquip>().module != null) {
      ...
      for (CartModel cartItem in cartRepo.getCartList()) {
        if (cartItem.item!.moduleId ==
            Get.find<EQSplashControllerEquip>().module!.id) {
      ...
      if (Get.find<EQSplashControllerEquip>().cacheModule != null) {
        ...
        for (CartModel cartItem in cartRepo.getCartList()) {
          if (cartItem.item!.moduleId ==
              Get.find<EQSplashControllerEquip>().cacheModule!.id) {
            _cartList.add(cartItem);
          }
        }
      }
  ...
  void addToCart(CartModel cartModel, int? index) {
    ...
    Get.find<EQItemController>().setExistInCart(cartModel.item, notify: true);
  ...
  void removeFromCart(int index) {
    ...
    cartRepo.addToCartList(_cartList);
    if (Get.find<EQItemController>().item != null) {
      Get.find<EQItemController>()
          .setExistInCart(Get.find<EQItemController>().item, notify: true);
    }
    calculationCart();
    update();
  }
