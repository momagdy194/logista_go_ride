...
import 'package:customer/equipment/data/model/response/rider_model.dart';
import 'package:customer/equipment/data/model/response/trip_model.dart';
...
import 'package:customer/equipment/helper/rider_type.dart';
import 'package:customer/equipment/util/images.dart';
import 'package:customer/equipment/view/base/custom_snackbar.dart';

class EQRiderController extends GetxController implements GetxService {
  final RiderRepo riderRepo;
  EQRiderController({required this.riderRepo});

  LatLng _initialPosition = LatLng(
    double.parse(Get.find<EQSplashControllerEquip>().configModel!.defaultLocation!.lat ?? '0'),
    double.parse(Get.find<EQSplashControllerEquip>().configModel!.defaultLocation!.lng ?? '0'),
  );
      ...
      builder: (BuildContext context, Widget? child) {
        return MediaQuery(
          data: MediaQuery.of(context).copyWith(
            alwaysUse24HourFormat: Get.find<EQSplashControllerEquip>().configModel!.timeformat == '24',
          ),
          child: child!,
        );
      },
  ...
  void initializeData(String? riderType, AddressModel? address) {
    ...
    _fromAddress = address ?? Get.find<EQLocationController>().getUserAddress();
  ...
  Future<Position?> getInitialLocation(AddressModel? address) async {
    ...
    try {
      ...
      final Uint8List liveMarkerIcon = await Get.find<EQRiderController>().getBytesFromAsset(Images.liveMarker, 70);
    ...
    if(response.statusCode == 200) {
      ...
      if(placeDetails.status == 'OK') {
        AddressModel address0 = AddressModel(
          ...
          contactPersonName: Get.find<EQLocationController>().getUserAddress()!.contactPersonName,
          contactPersonNumber: Get.find<EQLocationController>().getUserAddress()!.contactPersonNumber,
        );
        ZoneResponseModel response0 = await Get.find<EQLocationController>().getZone(address0.latitude, address0.longitude, false);
        if (response0.isSuccess) {
          if(Get.find<EQLocationController>().getUserAddress()!.zoneIds!.contains(response0.zoneIds[0])) {
  ...
  void setFromToMarker({required LatLng from, required LatLng to}) async {
    ...
    _distance = await Get.find<EQOrderController>().getDistanceInKM(from, to);
    _duration = await Get.find<EQOrderController>().getDistanceInKM(from, to, isDuration: true);
    if (kDebugMode) {
