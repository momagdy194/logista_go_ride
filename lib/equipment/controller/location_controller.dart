...
import 'package:customer/equipment/data/model/response/place_details_model.dart';
...
import 'package:customer/equipment/view/screens/location/widget/module_dialog.dart';
import 'package:customer/equipment/view/screens/location/widget/permission_dialog.dart';

class EQLocationController extends GetxController implements GetxService {
  final LocationRepo locationRepo;
  EQLocationController({required this.locationRepo});
  ...
  Future<AddressModel> getCurrentLocation(bool fromAddress, {GoogleMapController? mapController, LatLng? defaultLatLng, bool notify = true}) async {
    ...
    try {
      ...
      myPosition = Position(
        latitude: defaultLatLng != null ? defaultLatLng.latitude : double.parse(Get.find<EQSplashControllerEquip>().configModel!.defaultLocation!.lat ?? '0'),
        longitude: defaultLatLng != null ? defaultLatLng.longitude : double.parse(Get.find<EQSplashControllerEquip>().configModel!.defaultLocation!.lng ?? '0'),
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel, bool fromCheckout, bool fromRide, int? storeZoneId) async {
    ...
    if (response.statusCode == 200) {
      if(fromRide){
        ...
        responseModel = ResponseModel(false, Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.showRestaurantText!
            ? 'your_selected_location_is_from_different_zone'.tr : 'your_selected_location_is_from_different_zone_store'.tr);
      }else {
        getAddressList();
        Get.find<EQOrderController>().setAddressIndex(0);
        String? message = response.body["message"];
        responseModel = ResponseModel(true, message);
      }
    } else {
  ...
  void _setZoneData(AddressModel address, bool fromSignUp, String? route, bool canRoute, bool isDesktop) {
    Get.find<EQLocationController>().getZone(address.latitude, address.longitude, false).then((response) async {
  ...
  void autoNavigate(AddressModel? address, bool fromSignUp, String? route, bool canRoute, bool isDesktop) async {
    if (isDesktop && Get.find<EQSplashControllerEquip>().configModel!.module == null) {
      ...
      Get.find<EQSplashControllerEquip>().getModules(headers: header);
      if (Get.isDialogOpen!) {
        Get.back();
      }
  ...
  void saveData(AddressModel address, bool fromSignUp, String? route, bool canRoute, bool isDesktop) async {
    if(!GetPlatform.isWeb) {
      if (Get.find<EQLocationController>().getUserAddress() != null) {
        if(Get.find<EQLocationController>().getUserAddress()!.zoneIds != null) {
          for(int zoneID in Get.find<EQLocationController>().getUserAddress()!.zoneIds!) {
            FirebaseMessaging.instance.unsubscribeFromTopic('zone_${zoneID}_customer');
          }
        }else {
          FirebaseMessaging.instance.unsubscribeFromTopic('zone_${Get.find<EQLocationController>().getUserAddress()!.zoneId}_customer');
        }
      } else {
    ...
    await Get.find<EQLocationController>().saveUserAddress(address);
    if(Get.find<EQAuthController>().isLoggedIn()) {
      await Get.find<EQWishListController>().getWishList();
      Get.find<EQAuthController>().updateZone();
    }
    HomeScreen.loadData(true);
    Get.find<EQOrderController>().clearPrevData(address.zoneId);
    if(fromSignUp) {
  ...
  Future<bool> checkLocationActive() async {
    ...
    if(isActiveLocation) {
      ...
      double? distance = await Get.find<EQOrderController>().getDistanceInKM(
  ...
  Future<void> navigateToLocationScreen(String page, {bool offNamed = false, bool offAll = false}) async {
    ...
    if(!fromHome && Get.find<EQLocationController>().getUserAddress() != null) {
      Get.dialog(const CustomLoader(), barrierDismissible: false);
      Get.find<EQLocationController>().autoNavigate(
        Get.find<EQLocationController>().getUserAddress(), fromSignup, null, false, ResponsiveHelper.isDesktop(Get.context),
      );
    }else if(Get.find<EQAuthController>().isLoggedIn()) {
      Get.dialog(const CustomLoader(), barrierDismissible: false);
      await Get.find<EQLocationController>().getAddressList();
      Get.back();
      if(Get.find<EQLocationController>().addressList!.isEmpty) {
