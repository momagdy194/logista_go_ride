...
class EQWishListController extends GetxController implements GetxService {
  final WishListRepo wishListRepo;
  final ItemRepo itemRepo;
  EQWishListController({required this.wishListRepo, required this.itemRepo});
    ...
    if (response.statusCode == 200) {
      ...
      if(response.body['item'] != null){
        response.body['item'].forEach((item) async {
          if(item['module_type'] == null || !Get.find<EQSplashControllerEquip>().getModuleConfig(item['module_type']).newVariation!
      ...
      response.body['store'].forEach((store) async {
        if(Get.find<EQSplashControllerEquip>().module == null){
          ...
          if(s != null && Get.find<EQSplashControllerEquip>().module!.id == s.moduleId) {
