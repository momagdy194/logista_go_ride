...
import 'package:customer/equipment/data/model/response/landing_model.dart';
...
class EQSplashControllerEquip extends GetxController implements GetxService {
  final SplashRepo splashRepo;
  EQSplashControllerEquip({required this.splashRepo});
  ...
  void switchModule(int index, bool fromPhone) async {
    if(_module == null || _module!.id != _moduleList![index].id) {
        await Get.find<EQSplashControllerEquip>().setModule(_moduleList![index]);
        Get.find<EQCartController>().getCartData();
  ...
  void removeModule() {
    ...
    Get.find<EQCampaignController>().itemCampaignNull();
  }
