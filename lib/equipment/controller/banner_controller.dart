...
import 'package:customer/equipment/data/model/response/banner_model.dart';
...
class EQBannerController extends GetxController implements GetxService {
  final BannerRepo bannerRepo;

  EQBannerController({required this.bannerRepo});

  List<String?>? _bannerImageList;
  List<String?>? _taxiBannerImageList;
  List<String?>? _featuredBannerList;
  List<dynamic>? _bannerDataList;
  List<BannerModel1>? _bannerDataModel1List = [];
  List<BannerModel1>? _bannerDataModel2List = [];
  ...
  List<BannerModel1>? get bannerDataModel1List => _bannerDataModel1List;
  List<BannerModel1>? get bannerDataModel2List => _bannerDataModel2List;
    ...
    if(_bannerImageList == null || reload) {
      ...
      if (response.statusCode == 200) {
        ...
        for (var banner in bannerModel.banners!) {
          ...
          print("banner00 ${banner.title}");
          // if(){
            (_bannerDataModel1List)!.add(banner);
          // }

if(banner.is_main!=1&&!banner.title!.contains("..")){
  _bannerImageList!.add(banner.image);
}
if(banner.is_main!=1&& banner.title!.contains("..")){
  _bannerDataModel2List!.add(banner);
}
          // _bannerImageList!.where((element) =>   element.is_main == 1&&element.module_id==null)
          if (banner.item != null) {
