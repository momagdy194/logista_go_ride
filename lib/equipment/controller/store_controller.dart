...
import 'package:customer/equipment/data/model/response/cart_suggested_item_model.dart';
...
import 'package:customer/equipment/data/model/response/recommended_product_model.dart';
...
class EQStoreController extends GetxController implements GetxService {
  final StoreRepo storeRepo;
  EQStoreController({required this.storeRepo});
  ...
  Future<void> getFeaturedStoreList() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var zone in Get.find<EQLocationController>().getUserAddress()!.zoneData!) {
  ...
  void setCategoryList() {
    if(Get.find<EQCategoryController>().categoryList != null && _store != null) {
      _categoryList = [];
      _categoryList!.add(CategoryModel(id: 0, name: 'all'.tr));
      for (var category in Get.find<EQCategoryController>().categoryList!) {
  ...
  Future<void> initCheckoutData2(int? storeId) async {
    ...
    Get.find<EQOrderController>().clearPrevData(null);
    await Get.find<EQStoreController>().getStoreDetails(Store(id: storeId), false);
    Get.find<EQOrderController>().initializeTimeSlot(_store!);
  }
  ...
  Future<Store?> getStoreDetails(Store store, bool fromModule, {bool fromCart = false}) async {
    ...
    if(store.name != null) {
      ...
      if (response.statusCode == 200) {
        ...
        Get.find<EQOrderController>().initializeTimeSlot(_store!);
        if(!fromCart){
          Get.find<EQOrderController>().getDistanceInKM(
            LatLng(
              double.parse(Get.find<EQLocationController>().getUserAddress()!.latitude!),
              double.parse(Get.find<EQLocationController>().getUserAddress()!.longitude!),
            ),
        ...
        if(fromModule) {
          ...
          Get.find<EQOrderController>().clearPrevData(_store!.zoneId);
        }
      } else {
        ApiChecker.checkApi(response);
      }
      Get.find<EQOrderController>().setOrderType(
  ...
  void setCategoryIndex(int index) {
    ...
    _storeItemModel = null;
    getStoreItemList(_store!.id, 1, Get.find<EQStoreController>().type, false);
    update();
  }
