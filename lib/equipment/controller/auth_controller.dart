...
import 'package:customer/equipment/data/model/body/store_body.dart';
import 'package:customer/equipment/data/model/body/signup_body.dart';
import 'package:customer/equipment/data/model/body/social_log_in_body.dart';
import 'package:customer/equipment/data/model/response/delivery_man_vehicles.dart';
...
import 'package:customer/equipment/data/model/response/zone_model.dart';
...
class EQAuthController extends GetxController implements GetxService {
  final AuthRepo authRepo;
  EQAuthController({required this.authRepo}) {
  ...
  Future<ResponseModel> registration(SignUpBody signUpBody) async {
    ...
    if (response.statusCode == 200) {
      if(!Get.find<EQSplashControllerEquip>().configModel!.customerVerification!) {
  ...
  Future<ResponseModel> login(String? phone, String password, {bool alreadyInApp = false}) async {
    ...
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      if(Get.find<EQSplashControllerEquip>().configModel!.customerVerification! && response.body['is_phone_verified'] == 0) {
    ...
    if (response.statusCode == 200) {
      ...
      if(token != null && token.isNotEmpty) {
        if(Get.find<EQSplashControllerEquip>().configModel!.customerVerification! && response.body['is_phone_verified'] == 0) {
          ...
          authRepo.saveUserToken(response.body['token']);
          await authRepo.updateToken();
          Get.find<EQLocationController>().navigateToLocationScreen('sign-in');
        }
      }else {
  ...
  bool clearSharedData() {
    Get.find<EQSplashControllerEquip>().setModule(null);
  ...
  void setLocation(LatLng location) async {
    ZoneResponseModel response = await Get.find<EQLocationController>().getZone(
      location.latitude.toString(), location.longitude.toString(), false,
    );
    _storeAddress = await Get.find<EQLocationController>().getAddressFromGeocode(LatLng(location.latitude, location.longitude));
