...
class CustomNavigationDrawerState extends State<CustomNavigationDrawer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if(Get.find<EQSplashControllerEquip>().moduleList == null && !mounted ? ResponsiveHelper.isDesktop(context) : true) {
  ...
  Widget getWidget(context, widget, Color? backgroundColor, EQSplashControllerEquip splashController) {
    return MouseRegion(
      onEnter: (event) {
        setState(() {
          isCollapsed = false;
        ...
        child: Row(
          children: <Widget>[
            ClipRRect(
              ...
              child: CustomImage(
                image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.moduleImageUrl}/${widget.icon}',
