...
class MenuDrawerState extends State<MenuDrawer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if(Get.find<EQSplashControllerEquip>().configModel!.refundPolicyStatus == 1) {
      _menuList.add(Menu(icon: Images.refund, title: 'refund_policy'.tr, onTap: () {
        Get.offNamed(RouteHelper.getHtmlRoute('refund-policy'));
      }));
    }
    if(Get.find<EQSplashControllerEquip>().configModel!.cancellationPolicyStatus == 1) {
    ...
    if(Get.find<EQSplashControllerEquip>().configModel!.shippingPolicyStatus == 1) {
    ...
    if(Get.find<EQSplashControllerEquip>().configModel!.customerWalletStatus == 1) {
      _menuList.add(Menu(icon: Images.wallet, title: 'wallet'.tr, onTap: () {
        Get.offNamed(RouteHelper.getWalletRoute(true));
      }));
    }

    if(Get.find<EQSplashControllerEquip>().configModel!.loyaltyPointStatus == 1) {
      _menuList.add(Menu(icon: Images.loyal, title: 'loyalty_points'.tr, onTap: () {
        Get.offNamed(RouteHelper.getWalletRoute(false));
      }));
    }
    if(Get.find<EQSplashControllerEquip>().configModel!.refEarningStatus == 1) {
      _menuList.add(Menu(icon: Images.referCode, title: 'refer_and_earn'.tr, onTap: () {
        Get.offNamed(RouteHelper.getReferAndEarnRoute());
      }));
    }
    if(Get.find<EQSplashControllerEquip>().configModel!.toggleDmRegistration!) {
      _menuList.add(Menu(
        icon: Images.deliveryManJoin, title: 'join_as_a_delivery_man'.tr,onTap: (){
          Get.toNamed(RouteHelper.getDeliverymanRegistrationRoute());
      }));
    }
    if(Get.find<EQSplashControllerEquip>().configModel!.toggleStoreRegistration!) {
      _menuList.add(Menu(
        icon: Images.restaurantJoin, title: Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.showRestaurantText!
          ? 'join_as_a_restaurant'.tr : 'join_as_a_store'.tr,
        onTap: () => Get.toNamed(RouteHelper.getRestaurantRegistrationRoute()),
      ));
    }
    _menuList.add(Menu(icon: Images.logOut, title: Get.find<EQAuthController>().isLoggedIn() ? 'logout'.tr : 'sign_in'.tr, onTap: () {
      Get.back();
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Container(
              padding:   EdgeInsets.symmetric(vertical: Dimensions.paddingSizeLarge, horizontal: 25),
                      ...
                      child: Row(children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: index != _menuList.length-1 ? Theme.of(context).primaryColor : Get.find<EQAuthController>().isLoggedIn() ? Theme.of(context).colorScheme.error : Colors.green,
                          ),
