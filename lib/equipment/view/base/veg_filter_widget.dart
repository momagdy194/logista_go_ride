...
class VegFilterWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for(int i=0; i < Get.find<EQItemController>().itemTypeList.length; i++){
      entryList.add(PopupMenuItem<int>(value: i, child: Row(children: [
        Get.find<EQItemController>().itemTypeList[i] == type
        ...
        Text(
          Get.find<EQItemController>().itemTypeList[i].tr,
          style: robotoMedium.copyWith(color: Get.find<EQItemController>().itemTypeList[i] == type
              ? Theme.of(context).textTheme.bodyMedium!.color : Theme.of(context).disabledColor),
        ),
      ])));
    }

    return (Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.vegNonVeg!
    && Get.find<EQSplashControllerEquip>().configModel!.toggleVegNonVeg!) ? Padding(
      ...
      child: PopupMenuButton<dynamic>(
        ...
        onSelected: (dynamic value) => onSelected!(Get.find<EQItemController>().itemTypeList[value]),
