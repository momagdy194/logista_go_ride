...
import 'package:customer/equipment/view/screens/search/widget/search_result_widget.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:customer/equipment/view/screens/store/widget/bottom_cart_widget.dart';
...
class SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if(_isLoggedIn) {
      Get.find<EQSearchingController>().getSuggestedItems();
    }
    Get.find<EQSearchingController>().getHistoryList();
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if(Get.find<EQSearchingController>().isSearchMode) {
          return true;
        }else {
          Get.find<EQSearchingController>().setSearchMode(true);
          return false;
        }
      },
      child: Scaffold(
          ...
          body: SafeArea(child: Padding(
            ...
            child: GetBuilder<EQSearchingController>(builder: (searchController) {
              ...
              return Column(children: [
                ...
                widget.queryText!.isNotEmpty ? const SizedBox() : Center(child: SizedBox(width: Dimensions.webMaxWidth, child: Row(children: [
                  ...
                  Expanded(child: SearchField(
                    ...
                    hint: Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.showRestaurantText!
                    ...
                    child: SizedBox(width: Dimensions.webMaxWidth, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      (_isLoggedIn && searchController.suggestedItemList != null) ? searchController.suggestedItemList!.isNotEmpty ?  GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return InkWell(
                            onTap: () {
                              Get.find<EQItemController>().navigateToItemPage(searchController.suggestedItemList![index], context);
                            },
                            child: Container(
  ...
  void _actionSearch(bool isSubmit, String? queryText, bool fromHome) {
    if(Get.find<EQSearchingController>().isSearchMode || isSubmit) {
      if(queryText!.isNotEmpty) {
        Get.find<EQSearchingController>().searchData(queryText, fromHome);
      }else {
        showCustomSnackBar(Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.showRestaurantText!
            ? 'search_food_or_restaurant'.tr : 'search_item_or_store'.tr);
      }
    }else {
      List<double?> prices = [];
