...
import 'package:customer/equipment/view/screens/wallet/widget/history_item.dart';
import 'package:customer/equipment/view/screens/wallet/widget/wallet_bottom_sheet.dart';
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  void initCall(){
    if(Get.find<EQAuthController>().isLoggedIn()){
      ...
      Get.find<EQWalletController>().getWalletTransactionList('1', false, widget.fromWallet);

      Get.find<EQWalletController>().setOffset(1);

      scrollController.addListener(() {
        if (scrollController.position.pixels == scrollController.position.maxScrollExtent
            && Get.find<EQWalletController>().transactionList != null
            && !Get.find<EQWalletController>().isLoading) {
          int pageSize = (Get.find<EQWalletController>().popularPageSize! / 10).ceil();
          if (Get.find<EQWalletController>().offset < pageSize) {
            Get.find<EQWalletController>().setOffset(Get.find<EQWalletController>().offset + 1);
            if (kDebugMode) {
              print('end of the page');
            }
            Get.find<EQWalletController>().showBottomLoader();
            Get.find<EQWalletController>().getWalletTransactionList(Get.find<EQWalletController>().offset.toString(), false, widget.fromWallet);
          }
        }
      });
    }
        ...
        onPressed: (){
          Get.dialog(
            Dialog(backgroundColor: Colors.transparent, child: WalletBottomSheet(
                fromWallet: widget.fromWallet, amount: Get.find<EQUserController>().userInfoModel!.loyaltyPoint == null ? '0' : Get.find<EQUserController>().userInfoModel!.loyaltyPoint.toString(),
            )),
          );
        },
      ) : null,
      body: GetBuilder<EQUserController>(
          builder: (userController) {
            return isLoggedIn ? userController.userInfoModel != null ? SafeArea(
              child: RefreshIndicator(
                onRefresh: () async{
                  Get.find<EQWalletController>().getWalletTransactionList('1', true, widget.fromWallet);
                  Get.find<EQUserController>().getUserInfo();
                },
                child: SingleChildScrollView(
                  ...
                  child: FooterView(
                    child: SizedBox(
                      ...
                      child: GetBuilder<EQWalletController>(
...
class WalletShimmer extends StatelessWidget {
  final EQWalletController walletController;
        ...
        return Padding(
          ...
          child: Shimmer(
            ...
            child: Column(children: [
              ...
              Padding(padding:   EdgeInsets.only(top: Dimensions.paddingSizeLarge), child: Divider(color: Theme.of(context).disabledColor)),
            ],
            ),
          ),
        );
