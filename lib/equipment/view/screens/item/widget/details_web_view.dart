...
class DetailsWebView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<EQItemController>(builder: (itemController) {
      ...
      String? baseUrl = itemController.item!.availableDateStarts == null ? Get.find<EQSplashControllerEquip>().
      configModel!.baseUrls!.itemImageUrl : Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.campaignImageUrl;
                ...
                children: [
                  ...
                  Expanded(flex: 6, child: SingleChildScrollView(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ItemTitleView(item: itemController.item, inStock: Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.stock! && stock! <= 0),
                        ...
                        SizedBox(width: 400, child: Row(children: [
                          Expanded(flex:5, child: CustomButton(
                            buttonText: (Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.stock! && stock! <= 0) ? 'out_of_stock'.tr
                                : itemController.item!.availableDateStarts != null ? 'order_now'.tr : itemController.cartIndex != -1 ? 'update_in_cart'.tr : 'add_to_cart'.tr,
                            onPressed: (!Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.stock! || stock! > 0) ?  () {
                              if(itemController.item!.availableDateStarts != null) {
                                Get.toNamed(RouteHelper.getCheckoutRoute('campaign'), arguments: CheckoutScreen(
                                  storeId: null, fromCart: false, cartList: [cartModel],
                                ));
                              }else if (Get.find<EQCartController>().existAnotherStoreItem(cartModel!.item!.storeId, Get.find<EQSplashControllerEquip>().module!.id)) {
                                ...
                                if(itemController.cartIndex == -1) {
                                  Get.find<EQCartController>().addToCart(cartModel!, itemController.cartIndex);
                                }
                                showCustomSnackBar('item_added_to_cart'.tr, isError: false);
                              }
                            } : null,
                          ...
                          Expanded(
                            ...
                            child: Container(
                              ...
                              child: GetBuilder<EQWishListController>(
