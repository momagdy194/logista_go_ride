    ...
    return Scaffold(
      ...
      body: SafeArea(child: Center(child: Scrollbar(child: SingleChildScrollView(
        ...
        child: FooterView(child: Container(
          ...
          padding: context.width > 700 ?   EdgeInsets.all(Dimensions.paddingSizeDefault) : null,
          ...
          child: Column(children: [
            ...
            GetBuilder<EQUserController>(builder: (userController) {
              return GetBuilder<EQAuthController>(builder: (authBuilder) {
                return CustomButton(
  ...
  void _resetPassword() {
    ...
    if (password.isEmpty) {
      ...
      if(widget.fromPasswordChange) {
        UserInfoModel user = Get.find<EQUserController>().userInfoModel!;
        user.password = password;
        Get.find<EQUserController>().changePassword(user).then((response) {
        ...
        Get.find<EQAuthController>().resetPassword(widget.resetToken, '+${widget.number!.trim()}', password, confirmPassword).then((value) {
          if (value.isSuccess) {
            Get.find<EQAuthController>().login('+${widget.number!.trim()}', password).then((value) async {
              Get.find<EQLocationController>().navigateToLocationScreen('reset-password');
            });
          } else {
