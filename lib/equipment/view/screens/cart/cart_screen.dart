...
import 'package:customer/equipment/view/base/web_constrained_box.dart';
import 'package:customer/equipment/view/screens/cart/widget/cart_item_widget.dart';
...
class _CartScreenState extends State<CartScreen> {
  ...
  void initState() {
    ...
    if(Get.find<EQCartController>().cartList.isNotEmpty){
      if(Get.find<EQCartController>().addCutlery){
        Get.find<EQCartController>().updateCutlery(isUpdate: false);
      }
      Get.find<EQCartController>().setAvailableIndex(-1, isUpdate: false);
      Get.find<EQStoreController>().getCartStoreSuggestedItemList(Get.find<EQCartController>().cartList[0].item!.storeId);
      Get.find<EQStoreController>().getStoreDetails(Store(id: Get.find<EQCartController>().cartList[0].item!.storeId, name: null), false, fromCart: true);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<EQCartController>(
  ...
  Widget pricingView(EQCartController cartController, Item item){
    return Container(
      ...
      child: GetBuilder<EQStoreController>(
        builder: (storeController) {
          return Column(children: [

            Get.find<EQSplashControllerEquip>().getModuleConfig(item.moduleType).newVariation!
            ...
            Container(
              ...
              padding:   EdgeInsets.all(Dimensions.paddingSizeDefault),
              margin: ResponsiveHelper.isDesktop(context) ?   EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeDefault, vertical: Dimensions.paddingSizeSmall) : EdgeInsets.zero,
              child: Column(
            ...
            Padding(
              ...
              child: Column(children: [
                ...
                SizedBox(height: Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.addOn! ? 10 : 0),

                Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.addOn! ? Row(
...
class CheckoutButton extends StatelessWidget {
  final EQCartController cartController;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: GetBuilder<EQStoreController>(
        builder: (storeController) {
          if(Get.find<EQStoreController>().store != null && !Get.find<EQStoreController>().store!.freeDelivery! && Get.find<EQSplashControllerEquip>().configModel!.freeDeliveryOver != null){
            percentage = cartController.subTotal/Get.find<EQSplashControllerEquip>().configModel!.freeDeliveryOver!;
          }
          return Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              (storeController.store != null && !storeController.store!.freeDelivery!
                  && Get.find<EQSplashControllerEquip>().configModel!.freeDeliveryOver != null && percentage < 1)
              ? Column(children: [
                  Row(children: [
                    Image.asset(Images.percentTag, height: 20, width: 20),
                    const SizedBox(width: Dimensions.paddingSizeExtraSmall),

                    Text(PriceConverter.convertPrice(Get.find<EQSplashControllerEquip>().configModel!.freeDeliveryOver! - cartController.subTotal), style: robotoMedium.copyWith(color: Theme.of(context).primaryColor)),
                    const SizedBox(width: Dimensions.paddingSizeExtraSmall),
              ...
              CustomButton(buttonText: 'proceed_to_checkout'.tr, onPressed: () {
                ...
                if(!cartController.cartList.first.item!.scheduleOrder! && availableList.contains(false)) {
                  ...
                  if(Get.find<EQSplashControllerEquip>().module == null) {
                    int i = 0;
                    for(i = 0; i < Get.find<EQSplashControllerEquip>().moduleList!.length; i++){
                      if(cartController.cartList[0].item!.moduleId == Get.find<EQSplashControllerEquip>().moduleList![i].id){
                        break;
                      }
                    }
                    Get.find<EQSplashControllerEquip>().setModule(Get.find<EQSplashControllerEquip>().moduleList![i]);
                  }
