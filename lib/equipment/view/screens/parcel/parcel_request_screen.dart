...
import 'package:customer/equipment/view/screens/checkout/widget/payment_button.dart';
import 'package:customer/equipment/view/screens/checkout/widget/tips_widget.dart';
import 'package:customer/equipment/view/screens/parcel/widget/card_widget.dart';
...
class _ParcelRequestScreenState extends State<ParcelRequestScreen> {
  ...
  void initCall(){
    if(_isLoggedIn) {
      Get.find<EQParcelController>().getDistance(widget.pickedUpAddress, widget.destinationAddress);
      Get.find<EQParcelController>().setPayerIndex(0, false);
      for(ZoneData zData in Get.find<EQLocationController>().getUserAddress()!.zoneData!){
        if(zData.id ==  Get.find<EQLocationController>().getUserAddress()!.zoneId){
          _isCashOnDeliveryActive = zData.cashOnDelivery! && Get.find<EQSplashControllerEquip>().configModel!.cashOnDelivery!;
          _isDigitalPaymentActive = zData.digitalPayment! && Get.find<EQSplashControllerEquip>().configModel!.digitalPayment!;
          if(Get.find<EQParcelController>().payerIndex == 0) {
            Get.find<EQParcelController>().setPaymentIndex(_isCashOnDeliveryActive! ? 0 : _isDigitalPaymentActive! ? 1 : 2, false);
          }else{
            Get.find<EQParcelController>().setPaymentIndex(0, false);
          }

        }
      }
      if (Get.find<EQUserController>().userInfoModel == null) {
        Get.find<EQUserController>().getUserInfo();
      }
      Get.find<EQOrderController>().updateTips(
        Get.find<EQAuthController>().getDmTipIndex().isNotEmpty ? int.parse(Get.find<EQAuthController>().getDmTipIndex()) : 0, notify: false,
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GetBuilder<EQParcelController>(builder: (parcelController) {
        double charge = -1;
        if(parcelController.distance != -1 && _isLoggedIn && parcelController.extraCharge != null) {
          double parcelPerKmShippingCharge = widget.parcelCategory.parcelPerKmShippingCharge! > 0 ? widget.parcelCategory.parcelPerKmShippingCharge!
              : Get.find<EQSplashControllerEquip>().configModel!.parcelPerKmShippingCharge!;
          double parcelMinimumShippingCharge = widget.parcelCategory.parcelMinimumShippingCharge! > 0 ? widget.parcelCategory.parcelMinimumShippingCharge!
              : Get.find<EQSplashControllerEquip>().configModel!.parcelMinimumShippingCharge!;
            ...
            child: FooterView(child: SizedBox(width: Dimensions.webMaxWidth, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              CardWidget(child: Row(children: [
                ...
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: CustomImage(
                      image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.parcelCategoryImageUrl}'
              ...
              GetBuilder<EQOrderController>(builder: (orderController) {
                  return Container(
                    ...
                    padding:   EdgeInsets.symmetric(vertical: Dimensions.paddingSizeLarge, horizontal: Dimensions.paddingSizeSmall),
              ...
              SizedBox(height: (Get.find<EQSplashControllerEquip>().configModel!.dmTipsStatus == 1) ? Dimensions.paddingSizeExtraSmall : 0),
              ...
              (Get.find<EQSplashControllerEquip>().configModel!.customerWalletStatus == 1 && parcelController.payerIndex == 0) ? PaymentButton(
                icon: Images.wallet,
              ...
              CheckoutCondition(orderController: Get.find<EQOrderController>(), parcelController: parcelController, isParcel: true),
  ...
  void orderCallback(bool isSuccess, String? message, String orderID, int? zoneID, double orderAmount, double? maxCodAmount) {
    Get.find<EQParcelController>().startLoader(false);
    if(isSuccess) {
      if(Get.find<EQOrderController>().isDmTipSave){
        Get.find<EQAuthController>().saveDmTipIndex(Get.find<EQOrderController>().selectedTips.toString());
      }
      if(Get.find<EQParcelController>().paymentIndex == 1) {
        if(GetPlatform.isWeb) {
          Get.back();
          String? hostname = html.window.location.hostname;
          String protocol = html.window.location.protocol;
          String selectedUrl = '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&&customer_id=${Get.find<EQUserController>()
              .userInfoModel!.id}&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
          html.window.open(selectedUrl,"_self");
        } else{
          Get.offNamed(RouteHelper.getPaymentRoute(orderID, Get.find<EQUserController>().userInfoModel!.id, 'parcel', orderAmount, _isCashOnDeliveryActive));
        }
      }else {
        Get.offNamed(RouteHelper.getOrderSuccessRoute(orderID));
      }
      Get.find<EQOrderController>().updateTips(-1, notify: false);
    }else {
      showCustomSnackBar(message);
    }
  }

  Widget _bottomButton(EQParcelController parcelController, double charge) {
    return CustomButton(
