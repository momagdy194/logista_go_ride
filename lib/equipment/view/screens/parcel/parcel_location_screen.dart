...
import 'package:customer/equipment/view/screens/parcel/widget/parcel_view.dart';
...
class _ParcelLocationScreenState extends State<ParcelLocationScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    Get.find<EQParcelController>().setIsPickedUp(true, false);
    Get.find<EQParcelController>().setIsSender(true, false);
    ...
    if (Get.find<EQAuthController>().isLoggedIn()){
      if(Get.find<EQUserController>().userInfoModel == null){
        ...
        _senderNameController.text = Get.find<EQUserController>().userInfoModel != null ? '${Get.find<EQUserController>().userInfoModel!.fName!} ${Get.find<EQUserController>().userInfoModel!.lName!}' : '';
        _senderPhoneController.text = Get.find<EQUserController>().userInfoModel != null ? Get.find<EQUserController>().userInfoModel!.phone! : '';
      }else{
        _senderNameController.text = '${Get.find<EQUserController>().userInfoModel!.fName!} ${Get.find<EQUserController>().userInfoModel!.lName!}';
        _senderPhoneController.text = Get.find<EQUserController>().userInfoModel!.phone ?? '';
      }
    }
    ...
    _tabController?.addListener((){
      ...
      // }
      Get.find<EQParcelController>().setIsPickedUp(_tabController!.index == 0, false);
      Get.find<EQParcelController>().setIsSender(_tabController!.index == 0, true);
    });
  }
  ...
  Widget _bottomButton() {
    return GetBuilder<EQParcelController>(
            ...
            if( _tabController!.index == 0 ) {
              ...
              if(parcelController.destinationAddress == null) {
                ...
                Get.toNamed(RouteHelper.getParcelRequestRoute(
                  widget.category,
                  Get.find<EQParcelController>().pickupAddress!,
                  Get.find<EQParcelController>().destinationAddress!,
                ));
              }
           }
          },
  ...
  void _validateSender() {
    if(Get.find<EQParcelController>().pickupAddress == null) {
      ...
      AddressModel pickup = AddressModel(
        address: Get.find<EQParcelController>().pickupAddress!.address,
        additionalAddress: Get.find<EQParcelController>().pickupAddress!.additionalAddress,
        addressType: Get.find<EQParcelController>().pickupAddress!.addressType,
        contactPersonName: _senderNameController.text.trim(),
        contactPersonNumber: _senderPhoneController.text.trim(),
        latitude: Get.find<EQParcelController>().pickupAddress!.latitude,
        longitude: Get.find<EQParcelController>().pickupAddress!.longitude,
        method: Get.find<EQParcelController>().pickupAddress!.method,
        zoneId: Get.find<EQParcelController>().pickupAddress!.zoneId,
        id: Get.find<EQParcelController>().pickupAddress!.id,
      ...
      Get.find<EQParcelController>().setPickupAddress(pickup, true);
