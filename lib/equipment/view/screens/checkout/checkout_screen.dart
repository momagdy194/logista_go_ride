...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initCall() {
    if (Get.find<EQAuthController>().isLoggedIn()) {
      _streetNumberController.text =
          Get.find<EQLocationController>().getUserAddress()!.streetNumber ?? '';
      _houseController.text =
          Get.find<EQLocationController>().getUserAddress()!.house ?? '';
      _floorController.text =
          Get.find<EQLocationController>().getUserAddress()!.floor ?? '';

      Get.find<EQLocationController>().getZone(
          Get.find<EQLocationController>().getUserAddress()!.latitude,
          Get.find<EQLocationController>().getUserAddress()!.longitude,
          false,
      ...
      if (Get.find<EQLocationController>().addressList == null) {
        Get.find<EQLocationController>().getAddressList();
      }
      if (widget.storeId == null) {
        _cartList = [];
        widget.fromCart
            ? _cartList!.addAll(Get.find<EQCartController>().cartList)
            : _cartList!.addAll(widget.cartList!);
        Get.find<EQStoreController>()
            .initCheckoutData2(_cartList![0]!.item!.storeId);
      }
      if (widget.storeId != null) {
        Get.find<EQStoreController>().initCheckoutData2(widget.storeId);
        Get.find<EQStoreController>()
            .pickPrescriptionImage(isRemove: true, isCamera: false);
        Get.find<EQCouponController>().removeCouponData(false);
      }
      _isWalletActive =
          Get.find<EQSplashControllerEquip>().configModel!.customerWalletStatus == 1;
      ...
      _tipController.text = Get.find<EQOrderController>().selectedTips != -1
          ? AppConstants.tips[Get.find<EQOrderController>().selectedTips]
          : '';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    Module? module =
        Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module;
              ...
              return GetBuilder<EQStoreController>(builder: (storeController) {
                ...
                addressList.add(DropdownItem<int>(
                    ...
                    child: SizedBox(
                      ...
                      child: AddressWidget(
                        address:
                            Get.find<EQLocationController>().getUserAddress(),
                        fromAddress: false,
                        fromCheckout: true,
                      ),
                    )));
                ...
                if (storeController.store != null) {
                  for (ZoneData zData in Get.find<EQLocationController>()
                      .getUserAddress()!
                      .zoneData!) {
                    if (zData.id == storeController.store!.zoneId) {
                      ...
                      _isDigitalPaymentActive = zData.digitalPayment! &&
                          Get.find<EQSplashControllerEquip>()
                              ...
                              .digitalPayment!;
                      if (firstTime) {
                        Get.find<EQOrderController>().setPaymentMethod(
                ...
                return GetBuilder<EQCouponController>(
                  ...
                  return GetBuilder<EQOrderController>(
                    ...
                    double tax = 0;
                    bool taxIncluded =
                        Get.find<EQSplashControllerEquip>().configModel!.taxIncluded ==
                            1;
                    double addOns = 0;
                    double subTotal = 0;
  ...
  Widget topSection(
      EQStoreController storeController,
      double charge,
      double deliveryCharge,
      EQOrderController orderController,
      EQLocationController locationController,
      List<DropdownItem<int>> addressList,
      bool tomorrowClosed,
      bool todayClosed,
      Module? module,
  ...
  Widget bottomSection(
      ...
      double subTotal,
      double discount,
      EQCouponController couponController,
      bool taxIncluded,
      double tax,
      double deliveryCharge,
      EQStoreController storeController,
        ...
        Container(
          ...
          child:
            ...
            CheckoutCondition(
                ...
                parcelController: Get.find<EQParcelController>()),
          ]),
        ),
        ResponsiveHelper.isDesktop(context)
            ? Padding(
                padding:
                      EdgeInsets.only(top: Dimensions.paddingSizeLarge),
  ...
  void _callback(bool isSuccess, String? message, String orderID, int? zoneID,
    ...
    if (isSuccess) {
      ...
      if (!Get.find<EQOrderController>().showBottomSheet) {
        Get.find<EQOrderController>().showRunningOrders();
      }
      if (Get.find<EQOrderController>().isDmTipSave) {
        Get.find<EQAuthController>().saveDmTipIndex(
            Get.find<EQOrderController>().selectedTips.toString());
      }
      Get.find<EQOrderController>().stopLoader();
      HomeScreen.loadData(true);
      if (Get.find<EQOrderController>().paymentMethodIndex == 1) {
        if (GetPlatform.isWeb) {
          Get.back();
          String? hostname = html.window.location.hostname;
          String protocol = html.window.location.protocol;
          String selectedUrl =
              '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&&customer_id=${Get.find<EQUserController>().userInfoModel!.id}&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
          html.window.open(selectedUrl, "_self");
        } else {
          Get.offNamed(RouteHelper.getPaymentRoute(
              orderID,
              Get.find<EQUserController>().userInfoModel!.id,
              Get.find<EQOrderController>().orderType,
              amount,
              _isCashOnDeliveryActive));
        }
      } else {
