...
import 'package:customer/equipment/data/model/response/order_details_model.dart';
...
import 'package:customer/equipment/view/screens/chat/widget/image_dialog.dart';
import 'package:customer/equipment/view/screens/order/widget/cancellation_dialogue.dart';
import 'package:customer/equipment/view/screens/order/widget/delivery_details.dart';
import 'package:customer/equipment/view/screens/order/widget/order_item_widget.dart';
import 'package:customer/equipment/view/screens/parcel/widget/details_widget.dart';
import 'package:customer/equipment/view/screens/review/rate_review_screen.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:customer/equipment/view/screens/store/widget/review_dialog.dart';
...
class OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  void _loadData(BuildContext context, bool reload) async {
    await Get.find<EQOrderController>().trackOrder(widget.orderId.toString(), reload ? null : widget.orderModel, false);
    Get.find<EQOrderController>().timerTrackOrder(widget.orderId.toString());
    // if(widget.orderModel == null) {
    //   await Get.find<SplashController>().getConfigData(loadModuleData: true);
    // }
    Get.find<EQOrderController>().getOrderDetails(widget.orderId.toString());
  }
            ...
            Expanded(child: Scrollbar(child: SingleChildScrollView(
              ...
              child: FooterView(child: SizedBox(width: Dimensions.webMaxWidth, child: Stack(
                children: [
                  ...
                  Column(children: [
                    DateConverter.isBeforeTime(order.scheduleAt) && Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).newVariation!
                    ...
                    ]) : CustomImage(image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.storeCoverPhotoUrl}/${order.store!.coverPhoto}', height: 150, width: double.infinity) : const SizedBox(),

                    parcel ? (ongoing && order.orderStatus == 'pending') ? Image.asset(Images.pendingOrderDetails, height: 160, width: double.infinity) : Center(child: CustomImage(image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.parcelCategoryImageUrl}/${order.parcelCategory!.image}', height: 160)) : const SizedBox(),
                    ...
                    prescriptionOrder ? ongoing ? Image.asset(
                      order.orderStatus == 'pending' ? Images.pendingOrderDetails : (order.orderStatus == 'confirmed' || order.orderStatus == 'processing' || order.orderStatus == 'handover')
                        ...
                        : CustomImage(image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.storeCoverPhotoUrl}/${order.store!.coverPhoto}', height: 160, width: double.infinity)
                        : const SizedBox(),
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    SizedBox(height: DateConverter.isBeforeTime(order.scheduleAt) && Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).newVariation!
                    ...
                    Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Get.find<EQSplashControllerEquip>().configModel!.orderDeliveryVerification! ? const Divider(height: Dimensions.paddingSizeLarge) : const SizedBox(),
                        Get.find<EQSplashControllerEquip>().configModel!.orderDeliveryVerification! ? Row(children: [
                        ...
                        Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).newVariation!
                        ? Column(children: [
                          const Divider(height: Dimensions.paddingSizeLarge),
                          ...
                          order.orderStatus == 'refund_requested' ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            (order.refund != null && order.refund!.image != null && order.refund!.image!.isNotEmpty) ? InkWell(
                              onTap: () => showDialog(context: context, builder: (context) {
                                return ImageDialog(imageUrl: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.refundImageUrl}/${order.refund!.image!.isNotEmpty ? order.refund!.image![0] : ''}');
                              }),
                              child: CustomImage(
                                height: 40, width: 40, fit: BoxFit.cover,
                                image: order.refund != null ? '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.refundImageUrl}/${order.refund!.image!.isNotEmpty ? order.refund!.image![0] : ''}' : '',
                              ),
                            ) : const SizedBox(),
                          ]) : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    (Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).orderAttachment! && order.orderAttachment != null
                    && order.orderAttachment!.isNotEmpty) ? Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          child: GridView.builder(
                              ...
                              itemBuilder: (BuildContext context, index) {
                                return Padding(
                                  ...
                                  child: InkWell(
                                    onTap: () => openDialog(context, '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.orderAttachmentUrl}/${order.orderAttachment![index]}'),
                                    child: Center(child: ClipRRect(
                                      borderRadius: BorderRadius.circular(Dimensions.radiusSmall),
                                      child: CustomImage(
                                        image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.orderAttachmentUrl}/${order.orderAttachment![index]}',
                        ...
                        SizedBox(width: (Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).orderAttachment!
                    ...
                    Container(
                      ...
                      padding:   EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge, vertical: Dimensions.paddingSizeSmall),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(parcel ? 'parcel_category'.tr : Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).showRestaurantText!
                        ...
                        (parcel && order.parcelCategory == null) ? Text(
                        ...
                        ) : (!parcel && order.store == null) ? Center(child: Padding(padding:   EdgeInsets.symmetric(vertical: Dimensions.paddingSizeSmall),
                          child: Text('no_restaurant_data_found'.tr, maxLines: 1, overflow: TextOverflow.ellipsis,
                              style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall)),
                        )) : Row(children: [

                          ClipOval(child: CustomImage(
                            image: parcel ? '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.parcelCategoryImageUrl}/${order.parcelCategory!.image}'
                                : '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.storeImageUrl}/${order.store!.logo}',
                          ...
                          (Get.find<EQSplashControllerEquip>().configModel!.refundActiveStatus! && order.orderStatus == 'delivered' && !parcel
                    ...
                    order.deliveryMan != null ? Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          ...
                          ClipOval(child: CustomImage(
                            image: '${Get.find<EQSplashControllerEquip>().configModel!.baseUrls!.deliveryManImageUrl}/${order.deliveryMan!.image}',
                            height: 35, width: 35, fit: BoxFit.cover,
                          )),
                          const SizedBox(width: Dimensions.paddingSizeSmall),
                      ...
                      child: Column(
                        children: [
                          parcel ? const SizedBox() : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).addOn! ? Row(
                            ...
                            Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).addOn! ? Divider(
                            ...
                            SizedBox(height: Get.find<EQSplashControllerEquip>().getModuleConfig(order.moduleType).addOn! ? 10 : 0),
  ...
  Widget _bottomView(EQOrderController orderController, OrderModel order, bool parcel, double totalPrice) {
    return Column(children: [
          ...
          child: Row(children: [
            ...
            || order.orderStatus == 'processing' || order.orderStatus == 'handover'|| order.orderStatus == 'picked_up') ? Expanded(
              child: CustomButton(
                ...
                margin: ResponsiveHelper.isDesktop(context) ? null :   EdgeInsets.all(Dimensions.paddingSizeSmall),
                onPressed: () async{
                  _timer?.cancel();
      ...
      (order.orderStatus == 'delivered' && (parcel ? order.deliveryMan != null : (orderController.orderDetails!.isNotEmpty && orderController.orderDetails![0].itemCampaignId == null))) ? Center(
        child: Container(
          width: Dimensions.webMaxWidth,
          padding: ResponsiveHelper.isDesktop(context) ? null :   EdgeInsets.all(Dimensions.paddingSizeSmall),
          child: CustomButton(
      ...
      (order.orderStatus == 'failed' && Get.find<EQSplashControllerEquip>().configModel!.cashOnDelivery!) ? Center(
        child: Container(
          width: Dimensions.webMaxWidth,
