...
import 'package:customer/equipment/view/screens/order/widget/track_details_view.dart';
import 'package:customer/equipment/view/screens/order/widget/tracking_stepper_widget.dart';
...
class OrderTrackingScreenState extends State<OrderTrackingScreen> {
  ...
  void _loadData() async {
    await Get.find<EQOrderController>().trackOrder(widget.orderID, null, true);
    await Get.find<EQLocationController>().getCurrentLocation(true, notify: false, defaultLatLng: LatLng(
          ...
          GoogleMap(
            ...
            onMapCreated: (GoogleMapController controller) {
              ...
              setMarker(
                ...
                track.orderType == 'take_away' ? Get.find<EQLocationController>().position.latitude == 0 ? track.deliveryAddress : AddressModel(
                  latitude: Get.find<EQLocationController>().position.latitude.toString(),
                  longitude: Get.find<EQLocationController>().position.longitude.toString(),
                  address: Get.find<EQLocationController>().address,
  ...
  void setMarker(Store? store, DeliveryMan? deliveryMan, AddressModel? addressModel, bool takeAway, bool parcel, bool isRestaurant) async {
    try {
      ...
      store != null ? _markers.add(Marker(
        ...
        infoWindow: InfoWindow(
          title: parcel ? 'Receiver' : Get.find<EQSplashControllerEquip>().configModel!.moduleConfig!.module!.showRestaurantText! ? 'store'.tr : 'store'.tr,
